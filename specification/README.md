...
#### 4.1.2. Добавление [комментариев]("Дополнительное значение (безопасность приложений)")
...
![alt-текст](https://github.com/defex04/Javito/blob/AleksandrKirillov-patch-1/struct_v2.png)

Рис. 2. Прототип первоначальной версии пользовательского интерфейса

1. График изменений курса биткоина.
  a. Инструмент помогает отследить состояние курса биткоина по отношению к четырем валютам-фильтрам: USD, EUR, CNY и GBP;
  
  b. График строится за тот промежуток времени, который вводит пользователь в специальных графах выбора дат. минимальный интервал отображения изменений - 1 день;
  
  c. Система определяет максимальное и минимальное значение курса биткоина за тот промежуток времени, который выбирает пользователь. Критические точки максимума и минимума графика окрашиваются в зеленый и красный цвет соответственно.

2. Таблица, в которой отображаются данные последнего закрытия торгов биткоинами по отношению к валютам.

3. Таблица, которая прогнозирует поведение курса биткоина на следующий день. Состоит из 3-х столбцов:

  a. Валюта, по которой строится прогноз поведения;
  
  b. Прогноз роста/падения котировок на следующий календарный день. Зеленая стрелка сигнализирует о том, что алгоритм предсказывает рост курса, красная - его снижение; 
  
  c. Истинность алгоритма. В данном столбце пользователь может посмотреть корректность алгоритма предсказания. Если алгоритм угадал поведение графика (рост/падение) за прошлый день, то в ячейке выводится “win”, иначе “lose”;

4. На сайте системы любой пользователь может оставить свой  комментарий:

  a. Для публикации комментария необходимо ввести имя пользователя;
  
  b. Длина  комментария не превышает 500 символов;
  
  c. После публикации комментария, пользователь не имеет возможности удалять и изменять его;
  
  d. Комментарии отображаются для всех пользователей. 
